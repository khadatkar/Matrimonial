...
namespace GYMONE.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public ActionResult Login(UserDTO dto)
        {
            ...
            using (Db db = new Db())
            {
                if (db.Users.Any(x => x.Username.Equals(dto.Username) && x.Password.Equals(dto.Password)))
                {
                    isValid = true;
                }
            }
            ...
            else
            {
                ...
                using (Db db = new Db())
                {
                    obj = db.Users.FirstOrDefault(x => x.Username.Equals(dto.Username));
                    if (db.Users.Any(x => x.Id.Equals("1")))
                    {
                ...
                //Session["LoginType"] = obj.
                //FormsAuthentication.SetAuthCookie(login.username, model.RememberMe);
